...
export const loadAddress = ({ commit }) => {
  return new Promise((resolve, reject) => {
    request({
      url: usersurl+'/api/UmsMemberReceiveAddress/GetListAddress',
      method: 'get',
...
export const editAddress=({ commit },data)=>{
  return new Promise((resolve, reject) => {
    request({
      url: usersurl+'/api/UmsMemberReceiveAddress/EditAddress',
      method: 'put',
      data: qs.stringify(data),
     // headers:{'content-type': 'application/x-www-form-urlencoded'}    
  }).then(res=>{
...
export const login = ({ commit }, data) => {
  return new Promise((resolve, reject) => {
    ...
    if(res.data.success)
    {
      localStorage.setItem('loginInfo', JSON.stringify(res.data.response));
      console.log(res.data);
      setToken(res.data.responseJson.token);
